...
const SearchBar: React.FC = () => {
    ...
    return (
        <div className={`flex items-center gap-3 w-full max-w-xl ${user ? 'ml-[23rem]' : 'ml-[12rem]'}`}> 
            <div className="bg-backgroundElevatedBase rounded-full p-[0.7rem] mt-1 hover:scale-[1.04]">
            <Link to="/">
            <HoverCard>
                <HoverCardTrigger asChild>
                    <div className="cursor-pointer">
                        {isHome ? <GoHomeFill color="white" size={27} /> : <GoHome color="white" size={27} />}
                    </div>
                </HoverCardTrigger>
                <HoverCardContent side="bottom" align="center" className="p-1 bg-backgroundElevatedHighlight text-white text-sm font-medium rounded shadow-lg mt-4">
                    <p>Home</p>
                </HoverCardContent>
            </HoverCard>
